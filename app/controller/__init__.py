...
if mcim_config.file_cdn:
    ...
    async def get_modrinth_file(project_id: str, version_id: str, file_name: str):
        cache = aio_redis_engine.hget("file_cdn_modrinth", f'{project_id}/{version_id}/{file_name}')
        if cache:
            ...
            if file:
                if file.file_cdn_cached:
                    url = f'{mcim_config.alist_endpoint}/modrinth/{file.hashes.sha1}'
    ...
    async def get_curseforge_file(fileid1: str, fileid2: str, file_name: str):
        ...
        cache = aio_redis_engine.hget("file_cdn_curseforge", f'{fileid}/{file_name}')
        if cache:
            return RedirectResponse(url=cache)
        else:
            ...
            if file:
                if file.file_cdn_cached:
                    url = f'{mcim_config.alist_endpoint}/curseforge/{file_name}'
                    return RedirectResponse(url=url)
