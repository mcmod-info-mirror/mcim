...
class CurseForgeApi:
    ...
    async def get_all_games(index=1, ps=50):
        """
        ...
        :param ps: 要包含在响应中的项目数

        用法: `resp = <CurseForgeApi>.get_all_games(index, ps)`
        """
        return await cli("/games", params={"index": index, "pageSize": ps}).result

    async def get_game(gameid: int = 432):
        """
    ...
    async def get_game_version(gameid, index=1, ps=50):
        """
    ...
    async def get_mods(modids: List[int]) -> list:
        """
    ...
    async def get_file(modid: int, fileid: int):
        """
    ...
    async def post_files(fileids: List[int]):
        """
    ...
    async def get_file_download_url(fileid: int, modid: int):
        """
    ...
    async def get_fingerprints(fingerprints: List[int]):
        """
...
async def test():
    ...
    print(await cf.get_fingerprints([1667027305, 320753275]))